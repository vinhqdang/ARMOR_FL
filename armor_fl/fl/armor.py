...
Per-round pipeline:
  ...
  5. Hard exclusion once log E_pop_k^t >= log(1/alpha_pop); excluded clients
     enter a probation window, after which their population e-process resets
     and they re-enter at reduced trust.
...
class ArmorAggregator:
    ...
    def step(self, client_updates: list[ClientUpdate]) -> RoundResult:
        ...
        for u, dist in zip(active, dists):
            ...
            if rec.pop_eprocess.exceeds(cfg.alpha_pop) and not burn_in_active:
                rec.excluded = True
        ...
        return RoundResult(
            ...
            log_e_pop=log_e_pop, log_e_drift=log_e_drift, status=status,
