...
The present work builds directly on FedSE-1DSqueezeNet \citep{zhou2026fedse1dsqueezenet}, published in \emph{Cluster Computing}, whose backbone architecture we reimplement without modification (Section~\ref{sec:setup}); our contribution is entirely at the aggregation layer, and we adopt its federated hyperparameters (Table~\ref{tab:hyperparams}) as closely as computational constraints allow. Recent centralized work on these same datasets \citep{xu2025fewshot,zhang2025transformer,palani2026comparative,xue2025haehrl,sireesha2026lightweight,zhao2024gan} does not address federated poisoning robustness but establishes that the classification task is solvable to a high standard when data can be pooled centrally, useful context for the accuracy ceilings in Section~\ref{sec:results}.
