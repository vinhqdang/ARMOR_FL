...
\documentclass[pdflatex,iicol,sn-mathphys-num]{sn-jnl}% Math/Physical Sciences Numbered style, double-column
                                                       % ("iicol") per Cluster Computing's own submission guidelines
...
\title[Self-Referential Byzantine-Robust Federated Aggregation]{Self-Referential Anytime-Valid Byzantine-Robust Aggregation for Federated Intrusion Detection}
...
This ordering is reflected in wall-clock measurements (Table~\ref{tab:overhead}, Section~\ref{sec:results}.10): Krum is, in practice, the slowest of the four aggregators, not ARMOR-FL -- its aggregation-layer cost is small relative to local training time, which dominates total wall-clock time for all four rules. This is a relevant counterpoint to Section~\ref{sec:results}.6's headline finding that Krum wins on accuracy: the advantage does not come for free, and a latency-sensitive deployment may still prefer ARMOR-FL's cost profile even where its accuracy trails.

\section{Experimental Setup and Results}\label{sec:setup}\label{sec:results}
...
To isolate the effect of the three corrections in Section~\ref{sec:method} from the aggregation mechanism's design in general, we retain the full robustness grid from an earlier, uncorrected ARMOR-FL implementation, run under identical hyperparameters, datasets, attacks, and combinations. We refer to it as \textbf{ARMOR-FL (uncorrected)} in Section~\ref{sec:results}.10, reported exclusively as an ablation baseline against the corrected method -- not a competing method, and its numbers should be read only against the corrected version's, not Krum, FoolsGold, or FedAvg.
...
This is a relevant counterpoint to Section~\ref{sec:results}.6: Krum's accuracy advantage comes with a consistent computational cost -- 38\% higher aggregate wall-clock time -- that a latency-sensitive deployment may not want to pay, particularly since Section~\ref{sec:results}.8 shows the advantage narrows or reverses under the non-IID, lighter-poisoning conditions where ARMOR-FL is strongest.
...
\textbf{Supplementary information.} Full per-combination result and detection-statistic tables, a reproducibility checklist, a worked numerical example, a notation summary, and baseline aggregation pseudocode are provided as Online Resource 1 accompanying this article.
